...
const RTPPacket_1 = __importDefault(require("./transports/RTPPacket"));
...
class RTSPClient extends events_1.EventEmitter {
    ...
    async sendAudioBackChannel(audioChunk) {
        ...
        while (audioChunk.length > 0) {
            ...
            const channelInterleaved = this.setupResult.filter((value) => {
            ...
            */
            const rtpChannel = Number.parseInt(channelInterleaved.split('-')[0], 10);
            let interleavedHeader = Buffer.from([0x24]); // set '$'
            interleavedHeader = Buffer.concat([interleavedHeader, Buffer.from([rtpChannel])]);
