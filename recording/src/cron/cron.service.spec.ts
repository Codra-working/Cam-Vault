...
function getMockedProviders(
  module: TestingModule,
): [
  MockedObject<ConfigService>,
  MockedObject<RecordingService>,
  CronService,
  SchedulerRegistry,
] {
  const configService: MockedObject<ConfigService> = module.get(ConfigService);
  const recordingService: MockedObject<RecordingService> =
    module.get(RecordingService);
...
describe('CronService.addRecordingJob', () => {
  ...
  beforeEach(async () => {
    ...
    [configService, recordingService, cronService, schedulerRegistry] =
      getMockedProviders(module);
  ...
  test('should read recording job config values from configService', () => {
    ...
    expect(configService.get.mock.calls.map(([val]) => val)).toStrictEqual([
      'streams',
      'videoLen',
      'targetDirectory',
      'duration',
  ...
  test('registered cron job should call recordingService.record with configured values', async () => {
    cronService.addRecordingJob();
    const recordingJob = schedulerRegistry.getCronJob('recording_service');
    await recordingJob.fireOnTick();
    expect(recordingService.record).toHaveBeenNthCalledWith(
      1,
      'test stream',
      10,
      'test target',
