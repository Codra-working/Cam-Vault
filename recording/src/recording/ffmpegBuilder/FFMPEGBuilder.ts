...
import { isFormatInputPathObject, isRTSPURL, RTSPURL } from "src/common/types/types"
import { URL } from "url"
import { FFMPEGBuildStrategy, FFMPEGBuildContext, Codec } from "./FFMPEGBuilderStrategy"
...
export class FFMPEGBuilder {
    ...
    commit() {
        if (!this.inputStream || !this.outputStream) {
            throw new Error("Both input and output streams must be set before save()")
        }
    ...
    applyStrategy(strategy: FFMPEGBuildStrategy, context: FFMPEGBuildContext) {//interface를 정해야됨
...
function isVideoSource(val: unknown): val is VideoSource {
    if (val instanceof URL || isRTSPURL(val) || isFormatInputPathObject(val)) return true
    return false
}

function videoSourceToString(src: VideoSource): string {
